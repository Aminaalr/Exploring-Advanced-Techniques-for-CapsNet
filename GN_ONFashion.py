...
def loadDataset():
    ...
    x_train_resized_rgb = np.array([np.array(Image.fromarray(image).resize((75, 75))) for image in x_train])
    x_train_resized_rgb = np.repeat(x_train_resized_rgb[..., np.newaxis], 3, axis=-1)

    # Resize testing images to (32, 32) and convert to RGB
    x_test_resized_rgb = np.array([np.array(Image.fromarray(image).resize((75, 75))) for image in x_test])
